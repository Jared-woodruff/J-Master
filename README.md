...
spectrum, loudness-matched **REF** bypass (tap **R**; hold it for a
momentary compare), **LIM Δ** (hear only what the limiter removes), a
**MON** monitor matrix (mono fold-down, side solo, or a single channel,
metered exactly as heard and never exported), full **undo/redo**, and
**A/B snapshot slots**. Click any knob's readout to type an exact value,
press **?** for the complete keyboard map, and the whole console re-flows
cleanly down to a 720 px window instead of clipping.
...
shares the same zoom, grid, playhead and loudness lane. **SPLIT** stacks the
source above the processed master on one timeline, and **LOOP** (the L key,
or a double-click) cycles the detected section under the cursor while you
work the console:
...
- **Cover art:** click or drop an image (auto-scaled to a 1000 px JPEG) and
  it embeds natively everywhere: an ID3v2.3 APIC frame, a FLAC PICTURE
  block, Ogg Opus METADATA_BLOCK_PICTURE (the muxer spans pages when the
  art demands it), and a WAV `id3 ` chunk. Covers travel with `.jmaster`
...
| Cover art survives every container | APIC / PICTURE / METADATA_BLOCK_PICTURE parsed back from real renders; 657 KB art spans 13 Ogg pages and still decodes |
| Idle repaint cost | 3 s paused = 1 waveform draw + 0 spectrum draws (was ~180 / ~540) |
